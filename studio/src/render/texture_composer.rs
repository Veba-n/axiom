...
pub fn bake_texture_to_image(ctx: &egui::Context, comp: &ComposedTexture) -> egui::ColorImage {
    let font_size = 16.0; // Bake çözünürlüğü
    let font_id = egui::FontId::new(font_size, egui::FontFamily::Monospace);
    
    // Karakterlerin boyutunu alıyoruz
    let sample_galley = ctx.fonts(|f| f.layout_no_wrap("A".to_string(), font_id.clone(), egui::Color32::WHITE));
    let char_width = sample_galley.rect.width().ceil() as usize;
    let char_height = sample_galley.rect.height().ceil() as usize;
    ...
    ctx.fonts(|f| {
        ...
        for ty in 0..comp.height {
            for tx in 0..comp.width {
                let cell = comp.cell(tx, ty);
                if cell.ch == ' ' || !cell.visible {
                    continue;
                }

                let galley = f.layout_no_wrap(cell.ch.to_string(), font_id.clone(), egui::Color32::WHITE);
                if let Some(row) = galley.rows.first() {
                    if let Some(glyph) = row.glyphs.first() {
                        let uv = glyph.uv_rect;
                        
                        let min_x = uv.min[0] as usize;
                        let min_y = uv.min[1] as usize;
                        let max_x = uv.max[0] as usize;
                        let max_y = uv.max[1] as usize;

                        let glyph_w = max_x.saturating_sub(min_x);
                        let glyph_h = max_y.saturating_sub(min_y);

                        let dest_x = tx as usize * char_width;
                        let dest_y = ty as usize * char_height;

                        let fg = egui::Color32::from_rgb(cell.fg[0], cell.fg[1], cell.fg[2]);

                        for y in 0..glyph_h.min(char_height) {
                            for x in 0..glyph_w.min(char_width) {
                                let atlas_idx = (min_y + y) * atlas_w + (min_x + x);
                                if atlas_idx < atlas.pixels.len() {
                                    let coverage = atlas.pixels[atlas_idx];
                                    if coverage > 0.0 {
                                        let final_alpha = coverage * cell.alpha;
...
impl TextureCache {
    ...
    pub fn apply_3d_transform(&mut self, texture_id: &str, _rotation: [f32; 3], scale: [f32; 3]) {
        if let Some((_, tex, _)) = self.entries.get_mut(texture_id) {
    ...
    pub fn get_or_compose(&mut self, ctx: &egui::Context, texture: &AxiomTexture) -> &ComposedTexture {
        let needs_recompute = match self.entries.get(&texture.id) {
            Some((cached_tex, _, _)) => cached_tex != texture,
            None => true,
        };
        if needs_recompute {
            println!("TEXTURE RECOMPUTED (BAKED): {}", texture.id);
            let comp = compose(texture);
            ...
            self.entries.insert(texture.id.clone(), (texture.clone(), comp, Some(handle)));
        }
        &self.entries.get(&texture.id).unwrap().1
    }
    
    pub fn get_handle(&self, texture_id: &str) -> Option<&egui::TextureHandle> {
        self.entries.get(texture_id).and_then(|(_, _, h)| h.as_ref())
    }

    pub fn sync(&mut self, ctx: &egui::Context, textures: &[AxiomTexture]) {
        for tex in textures {
            self.get_or_compose(ctx, tex);
        }
        let valid_ids: std::collections::HashSet<&str> = textures.iter().map(|t| t.id.as_str()).collect();
        self.entries.retain(|id, _| valid_ids.contains(id.as_str()));
    }

    pub fn get(&self, texture_id: &str) -> Option<&ComposedTexture> {
        self.entries.get(texture_id).map(|(_, c, _)| c)
    }
}
