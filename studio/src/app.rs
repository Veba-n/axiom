...
pub struct AxiomStudio {
    ...
    pub last_engine_frame_time: f64,
    pub current_fps: f32,
    pub current_engine_fps: f32,
    
    // WGPU Cihaz Bilgileri
    pub active_gpu_name: String,
    pub gpu_preference: String, // "HighPerformance" veya "LowPower"
...
impl AxiomStudio {
    ...
    pub fn push_history(&mut self) {
        let should_push = if let Some(last) = self.history.get(self.history_index) {
            // SIFIR-TAHSİS (ZERO-ALLOC) KARŞILAŞTIRMA! 
            // Koca bir sahneyi her 1 saniyede bir allocate edip klonlamaktan kurtarır.
            last.objects != self.objects ||
            last.textures != self.textures ||
            last.settings != self.settings ||
            last.scenes != self.scenes ||
            last.active_scene != self.active_scene ||
            last.windowed_panels != self.windowed_panels
        } else {
            true
        };
            ...
            std::thread::spawn(move || {
                if let Ok(json) = serde_json::to_string_pretty(&data) {
                    if std::path::Path::new(&path_clone).exists() {
                        let bak_path = format!("{}.bak", path_clone);
                        std::fs::copy(&path_clone, &bak_path).ok();
                    }
                    
                    let tmp_path = format!("{}.tmp", path_clone);
                    if std::fs::write(&tmp_path, &json).is_ok() {
                        std::fs::rename(&tmp_path, &path_clone).ok();
                    }
                }
            });
        }
...
impl Default for AxiomStudio {
    fn default() -> Self {
        ...
        Self {
            ...
            show_fps: false,
            last_frame_time: 0.0, last_interaction_time: 0.0,
            last_engine_frame_time: 0.0,
            current_fps: 0.0,
            current_engine_fps: 0.0,
            active_gpu_name: String::new(),
            gpu_preference: String::new(),
            history: vec![],
            history_index: 0,
...
impl eframe::App for AxiomStudio {
    fn update(&mut self, ctx: &eframe::egui::Context, _frame: &mut eframe::Frame) {
        ...
        // PERFORMANS: Sürükleme (Drag) anında 1.5ms'lik history droplarını engelle!
        // Sadece fare tuşu bırakıldığında veya klavye girişi olduğunda history kaydet.
        let is_dragging = ctx.input(|i| i.pointer.any_down());
        let input_released = ctx.input(|i| i.pointer.any_released() || (!i.events.is_empty() && !i.pointer.any_down()));
        
        if input_released {
            self.push_history();
            self.last_history_check_time = time_f64;
        } else if !is_dragging && time_f64 - self.last_history_check_time > 5.0 {
            // Eğer mouse bırakılmadıysa ama 5 saniye geçtiyse ve sürükleme yoksa yedekle
            self.push_history();
            self.last_history_check_time = time_f64;
        }
        ...
        if self.last_frame_time > 0.0 {
            ...
            if dt > 0.0 {
                ...
                if self.current_fps < 1.0 || self.current_fps.is_nan() {
                    self.current_fps = instant_fps;
                } else {
                    self.current_fps = self.current_fps * 0.90 + instant_fps * 0.10;
                }
            }
        }
        
        // Motor FPS yumuşatma (EMA)
        if self.last_engine_frame_time > 0.0 {
            let instant_engine_fps = (1.0 / self.last_engine_frame_time) as f32;
            if self.current_engine_fps < 1.0 || self.current_engine_fps.is_nan() {
                self.current_engine_fps = instant_engine_fps;
            } else {
                self.current_engine_fps = self.current_engine_fps * 0.95 + instant_engine_fps * 0.05;
            }
        }
        ...
        if self.show_fps {
            egui::Area::new(egui::Id::new("fps_counter_area"))
                ...
                .show(ctx, |ui| {
                    ui.label(
                        egui::RichText::new(format!("Limitli FPS: {:.1}\nMotor FPS Kapasitesi: {:.0}", self.current_fps, self.current_engine_fps))
