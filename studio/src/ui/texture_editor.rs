...
pub fn show(app: &mut AxiomStudio, ctx: &egui::Context) {
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        ...
        ui.columns(2, |cols| {
            ...
            let composed = app
                .texture_cache
                .get_or_compose(&left_ui.ctx().clone(), &app.textures[app.active_texture_index])
                .clone();
            let handle = app.texture_cache.get_handle(&app.textures[app.active_texture_index].id);
            draw_preview(right_ui, &composed, &app.textures[app.active_texture_index], app.texture_preview_mode, handle);
        });
    });
    ...
    if preview_mode == 3 {
        ...
        for (face_indices, face_name) in faces {
            ...
            let (obj_scale_x, obj_scale_y) = if face_name.contains("Z") {
                (cube_scale_x, cube_scale_y)
            } else if face_name.contains("X") {
                (cube_scale_z, cube_scale_y)
            } else if face_name.contains("Y") {
                (cube_scale_x, cube_scale_z)
            } else {
                (1.0, 1.0)
            };

            let base_density = (composed.width.max(composed.height).max(1)) as f32;
            let uv_mul = uv_scale_x.max(uv_scale_y).max(0.01);
            let density = base_density * uv_mul;

            let face_w = if auto_tile { obj_scale_x.abs() } else { 1.0 };
            let face_h = if auto_tile { obj_scale_y.abs() } else { 1.0 };

            let tw = composed.width.max(1) as f32;
            let th = composed.height.max(1) as f32;

            let uv_scale_x_final = density * face_w / tw;
            let uv_scale_y_final = density * face_h / th;

            let uv_0 = egui::pos2(uv_offset_x, uv_offset_y);
            let uv_1 = egui::pos2(uv_offset_x + uv_scale_x_final, uv_offset_y);
            let uv_2 = egui::pos2(uv_offset_x + uv_scale_x_final, uv_offset_y + uv_scale_y_final);
            let uv_3 = egui::pos2(uv_offset_x, uv_offset_y + uv_scale_y_final);

            let color = egui::Color32::WHITE;

            let mut mesh = egui::Mesh::default();
            if let Some(tex_handle) = texture_handle {
                 mesh = egui::Mesh::with_texture(tex_handle.id());
            }

            mesh.vertices.push(egui::epaint::Vertex { pos: v0, uv: uv_0, color });
            mesh.vertices.push(egui::epaint::Vertex { pos: v1, uv: uv_1, color });
            mesh.vertices.push(egui::epaint::Vertex { pos: v2, uv: uv_2, color });
            mesh.vertices.push(egui::epaint::Vertex { pos: v3, uv: uv_3, color });
